...
export class NocoDBClient {
  ...
  async getBase(baseId: string): Promise<NocoDBBase> {
    const response = await this.client.get(`/api/v1/db/meta/projects/${baseId}`);
    return response.data;
  }

  // Table operations
  async listTables(baseId: string): Promise<NocoDBTable[]> {
    const response = await this.client.get(`/api/v1/db/meta/projects/${baseId}/tables`);
    return response.data.list;
  }
  ...
  async createTable(baseId: string, tableName: string, columns: any[]): Promise<NocoDBTable> {
    const response = await this.client.post(`/api/v1/db/meta/projects/${baseId}/tables`, {
  ...
  async updateRecord(
    ...
    const pkColumn = columns.find(col => col.pk) || columns.find(col => col.title === 'ID');
  ...
  async createView(tableId: string, title: string, type: number = 1): Promise<NocoDBView> {
  ...
  async groupBy(
    ...
    if (options?.sort) {
      const sortField = Array.isArray(options.sort) ? options.sort[0] : options.sort;
